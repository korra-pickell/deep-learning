...
save_dir = 'E:\\DATA\\NPY\\3d-object-removal'

label = 'train'

if label == 'train':
    wdir_origin = ['E:\\DATA\\3d-object-removal\\original\\']
    wdir_offset = ['E:\\DATA\\3d-object-removal\\removed\\']
else:
    wdir_origin = ['E:\\DATA\\3d-object-removal\\demo_orig\\']
    wdir_offset = ['E:\\DATA\\3d-object-removal\\demo_rem\\']
...
for chunk_index,chunk in enumerate([raw_images_origin[i:i+chunk_length] for i in range(0,len(raw_images_origin),chunk_length)]):
    ...
    np.savez_compressed(save_dir+'/'+'object-removal-'+str(label)+'-'+str(w)+'-'+str(chunk_index)+'.npz',x=np.array(x_data),y=np.array(y_data))
    print('')
