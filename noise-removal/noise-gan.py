...
def generator_loss(disc_generated_output, gen_output, target):
    ...
    # mean absolute error
    #print('TYPE --------------------')
    #print(target.dtype,gen_output.dtype)
    #tf.cast(target, tf.float64)
    #tf.cast(gen_output, tf.float64)
    #print(target.dtype,gen_output.dtype)
...
def generate_images(model, test_input, tar, file_index, epoch):
    ...
    for i in range(columns*rows):
        ...
        plt.imshow(img)
        plt.axis('off')
    plt.savefig('E:/DATA/img_output/noise/output_'+str(epoch)+'_'+str(file_index)+'.png')
    plt.close()
...
if file_break == True:
    for epoch_number in range(epoch_count):
        for file_index,file in enumerate(training_files):
            file_loaded = np.load(file,allow_pickle=True)
            train_dataset_np_x = file_loaded['x']
            train_dataset_np_y = file_loaded['y'].astype('float32')
    ...
    arrays_x = []
    arrays_y = []
    for index,file in enumerate(training_files_x):
        print(index+1)
        arrays_x.append(np.load(file,allow_pickle=True))
    for index,file in enumerate(training_files_y):
        print(index+1)
        arrays_y.append(np.load(file,allow_pickle=True).astype('float32'))
