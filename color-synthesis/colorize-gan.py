...
def generate_images(model, test_input, tar, file_index, epoch):
    ...
    for i in range(columns*rows):
        ...
        plt.imshow(img)
        plt.axis('off')
    plt.savefig('E:/DATA/img_output/colorize/output_'+str(epoch)+'_'+str(file_index)+'.png')
    plt.close()
...
test_dataset_x = np.load(test_files_x[0],allow_pickle=True)['arr_0']
test_dataset_y = np.load(test_files_y[0],allow_pickle=True)['arr_0']

test_dataset = tf.data.Dataset.from_tensor_slices((test_dataset_x,test_dataset_y))#.batch(BATCH_SIZE)


#If file_break == True, then load each file seperately to save system resources (Recommended)
if file_break == True:
    for epoch_number in range(epoch_count):
        for file_index,file in enumerate(training_files_x):

            train_dataset_np_x = np.load(file,allow_pickle=True)['arr_0']
            
            train_dataset_np_y = np.load(training_files_y[file_index],allow_pickle=True)['arr_0'].astype('float32')
    ...
    arrays_x = [np.load(file,allow_pickle=True)['arr_0'] for file in training_files_x]
    arrays_y = [np.load(file,allow_pickle=True)['arr_0'].astype('float32') for file in training_files_y]
